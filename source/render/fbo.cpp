    ...
    if(status != GL_FRAMEBUFFER_COMPLETE_EXT) {
        switch(status) {
            case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT_EXT:
                kexlib::system->Warning("kexFBO::Init: bad attachment\n");
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT_EXT:
                kexlib::system->Warning("kexFBO::Init: attachment is missing\n");
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
                kexlib::system->Warning("kexFBO::Init: bad dimentions\n");
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT:
                kexlib::system->Warning("kexFBO::Init: bad format\n");
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER_EXT:
                kexlib::system->Warning("kexFBO::Init: error with draw buffer\n");
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER_EXT:
                kexlib::system->Warning("kexFBO::Init: error with read buffer\n");
                break;
            default:
                kexlib::system->Warning("kexFBO::Init: frame buffer creation didn't complete\n");
                break;
        }
...
void kexFBO::InitColorAttachment(const int attachment, const int width, const int height) {
    ...
    if(attachment < 0 || attachment >= kexlib::render::backend->MaxColorAttachments()) {
        return;
    }
    ...
    kexlib::render::backend->RestoreFrameBuffer();
}
...
void kexFBO::InitColorAttachment(const int attachment) {
    int width = kexMath::RoundPowerOfTwo(kexlib::system->VideoWidth());
    int height = kexMath::RoundPowerOfTwo(kexlib::system->VideoHeight());
...
void kexFBO::CopyBackBuffer(void) {
    int viewWidth = kexMath::RoundPowerOfTwo(kexlib::system->VideoWidth());
    int viewHeight = kexMath::RoundPowerOfTwo(kexlib::system->VideoHeight());
...
void kexFBO::CopyFrameBuffer(const kexFBO &fbo) {
    int viewWidth = kexlib::system->VideoWidth();
    int viewHeight = kexlib::system->VideoHeight();
...
void kexFBO::Bind(void) {
    if(fboId == kexlib::render::backend->glState.currentFBO) {
        return;
    }
    ...
    dglDrawBuffer(fboAttachment);
    kexlib::render::backend->glState.currentFBO = fboId;
}
...
void kexFBO::UnBind(void) {
    if(kexlib::render::backend->glState.currentFBO == 0) {
        return;
    }
...
void kexFBO::BindImage(void) {
    int unit = kexlib::render::backend->glState.currentUnit;
    dtexture currentTexture = kexlib::render::backend->glState.textureUnits[unit].currentTexture;
    ...
    dglBindTexture(GL_TEXTURE_2D, fboTexId);
    kexlib::render::backend->glState.textureUnits[unit].currentTexture = fboTexId;
}
...
void kexFBO::UnBindImage(void) {
    ...
    if(kexlib::render::backend->glState.textureUnits[unit].currentTexture == 0) {
        return;
    }
    
    dglBindTexture(GL_TEXTURE_2D, 0);
    kexlib::render::backend->glState.textureUnits[unit].currentTexture = 0;
}
