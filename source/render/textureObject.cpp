...
void kexTexture::SetParameters(void) {
    ...
    if(has_GL_EXT_texture_filter_anisotropic) {
        if(cvarGLAnisotropic.GetInt()) {
            dglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                kexlib::render::backend->MaxAnisotropic());
        }
        else {
...
void kexTexture::Upload(kexImageManager &image, texClampMode_t clamp, texFilterMode_t filter) {
    ...
    if(!kexlib::render::backend->IsInitialized()) {
        return;
    }
...
void kexTexture::Bind(void) {
    ...
    if(bLoaded == false) {
        ...
        if(bLoaded == false || texid == 0) {
            ...
            tid = *kexlib::render::backend->defaultTexture.TextureID();
        }
        else {
            tid = texid;
        }
    ...
    kexlib::render::backend->glState.textureUnits[unit].currentTexture = tid;
    kexlib::render::backend->glState.numTextureBinds++;
}
...
void kexTexture::BindFrameBuffer(const bool bReadBuffer) {
    ...
    if(texid != currentTexture) {
        dglBindTexture(GL_TEXTURE_2D, texid);
        kexlib::render::backend->glState.textureUnits[unit].currentTexture = texid;
    }
    ...
    origwidth   = kexlib::system->VideoWidth();
    origheight  = kexlib::system->VideoHeight();
    width       = origwidth;
    height      = origheight;
