...
void kexImageManager::LoadFromFile(const char *file) {
    ...
    if(kexlib::fileSystem->OpenFile(file, &fileData, hb_static) == 0) {
        return;
    }
    ...
    if(strstr(file, ".tga")) {
        ...
        data = NULL;
        kexlib::system->Warning("kexImageManager::LoadFromFile(%s) - Unknown file format\n", file);
    }
...
void kexImageManager::LoadFromScreenBuffer(void) {
    ...
    origwidth   = kexlib::render::backend->ViewWidth();
    origheight  = kexlib::render::backend->ViewHeight();
    width       = origwidth;
    height      = origheight;
    x           = kexlib::render::backend->WindowX();
    y           = kexlib::render::backend->WindowY();
    col         = (width * 3);
    colorMode   = TCR_RGB;
...
void kexImageManager::LoadFromTGA(byte *input) {
    ...
    tmp[0]          = rover[0];
    tmp[1]          = rover[1];
    tga.cmap_start  = kexlib::system->SwapLE16(*((short*)tmp));
    rover += 2;
    tmp[0]          = rover[0];
    tmp[1]          = rover[1];
    tga.cmap_len    = kexlib::system->SwapLE16(*((short*)tmp));
    rover += 2;
    tga.cmap_bits   = *rover++;
    tga.xorigin     = kexlib::system->SwapLE16(*((short*)rover));
    rover += 2;
    tga.yorigin     = kexlib::system->SwapLE16(*((short*)rover));
    rover += 2;
    tga.width       = kexlib::system->SwapLE16(*((short*)rover));
    rover += 2;
    tga.height      = kexlib::system->SwapLE16(*((short*)rover));
    rover += 2;
    ...
    case TGA_TYPE_INDEXED:
        ...
        case 8:
            ...
            case 32:
                ...
                break;
            default:
                kexlib::system->Error("%i-bit color map not supported for %s", tga.cmap_bits, filePath);
                break;
            }
            break;
        case 24:
            kexlib::system->Error("24 bits (indexed) is not supported for %s", filePath);
        case 32:
            kexlib::system->Error("32 bits (indexed) is not supported for %s", filePath);
            break;
        default:
            kexlib::system->Error("unknown pixel bit for %s", filePath);
            break;
        }
        break;
    case TGA_TYPE_RGB:
        ...
        break;
    case TGA_TYPE_BW:
        kexlib::system->Error("grayscale images is not supported for %s", filePath);
        break;
    case TGA_TYPE_RLE_INDEXED:
        kexlib::system->Error("RLE indexed images is not supported for %s", filePath);
        break;
    case TGA_TYPE_RLE_RGB:
        kexlib::system->Error("RLE images is not supported for %s", filePath);
        break;
    case TGA_TYPE_RLE_BW:
        kexlib::system->Error("RLE grayscale images is not supported for %s", filePath);
        break;
    default:
        kexlib::system->Error("%s has unknown tga type", filePath);
        break;
    }
}
...
void kexImageManager::LoadFromBMP(byte *input) {
    ...
    bmp.id[0]       = *rover++;
    bmp.id[1]       = *rover++;
    bmp.fileSize    = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.u1          = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.dataOffset  = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.headerSize  = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.width       = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.height      = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.planes      = kexlib::system->SwapLE16(*(short*)rover); rover += 2;
    bmp.bits        = kexlib::system->SwapLE16(*(short*)rover); rover += 2;
    bmp.compression = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.dataSize    = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.hRes        = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.vRes        = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.colors1     = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    bmp.colors2     = kexlib::system->SwapLE32(*(long*)rover); rover += 4;
    ...
    if(bmp.id[0] != 'B' && bmp.id[1] != 'M') {
        kexlib::system->Error("bitmap (%s) has unknown header ID ('BM' only supported\n", filePath);
    }
    if(bmp.compression != 0) {
        kexlib::system->Error("compression not supported for bitmap (%s)\n", filePath);
    }
    if(bmp.bits < 8) {
        kexlib::system->Error("monochrome and 4-bit pixels not supported for bitmap (%s)\n", filePath);
    }
        ...
        for(int x = 0; x < cols; x++) {
            ...
            case 32:
                ...
                *buf++ = rgba[3];
                break;
            default:
                kexlib::system->Error("bitmap (%s) has unknown pixel format (%i)\n", filePath, bmp.bits);
                break;
            }
        }
    }
